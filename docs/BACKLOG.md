...
> **Versão:** 1.8 | **Atualizado em:** 2026-06-29 (sessão 46)
...
| PROD-04 | ~~CPF pré-preenchido no checkout Asaas~~ | ✅ | **Resolvido (sessão 43).** Root cause: `findOrCreateCustomer` criava o customer no Asaas com `{ name, email }` apenas — CPF ia só no `creditCardHolderInfo` do payment (cobrança API). Asaas usa dados do *customer* para pré-preencher o checkout. Fix: `findOrCreateCustomer` agora passa `cpfCnpj + phone + postalCode + addressNumber` no `POST /customers`. Customers existentes sem CPF são atualizados via `PUT /customers/{id}`. **Regressão corrigida (sessão 46):** quando `asaas_customer_id` salvo no banco era reutilizado diretamente (bypass de `findOrCreateCustomer`), o customer Asaas nunca recebia atualização de CPF/phone caso o usuário tivesse preenchido o perfil após o primeiro checkout. Fix: `PUT /customers/{id}` de sync adicionado ao path de reutilização (commit `13384422`). |
| PROD-05 | ~~Pagamentos duplicados / pendentes no Asaas~~ | ✅ | **Resolvido (sessão 43) + Regressão 1 corrigida (sessão 44) + Regressão 2 corrigida (sessão 46).** Problema original: cada clique em "Assinar" criava novo payment no Asaas. Fix sessão 43: check idempotente via `GET /payments/{asaas_payment_id}` antes de criar novo. **Regressão 1 sessão 43:** `select` incompleto tornava os blocos de idempotência dead code — corrigido em sessão 44. **Regressão 2 sessão 45/46:** verificação `subRow?.status === "pending_payment"` na condição de idempotência bloqueava o reaproveitamento para usuários retornantes (status `"trialing"` / `"canceled"` / `"active"` nunca iguala `"pending_payment"`). Fix: condição `status` removida da idempotência — Asaas é a fonte de verdade via `GET /payments/{id}` (commit `13384422`). |
