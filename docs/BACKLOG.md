...
> **Versão:** 2.7 | **Atualizado em:** 2026-07-02 (sessão 71 — BK-11/ID-016 Zod schemas para PHI concluído)
...
| BK-11 | ~~Zod schemas para formulários com PHI~~ | ✅ | **Implementado (sessão 71, 2026-07-02, commit `3c72cb2`).** `src/lib/schemas/{auth,consultation,surgery,medication}.ts` criados. Cadastro, Login, AddConsultationDrawer e AddSurgeryDrawer migrados para `react-hook-form` + `zodResolver`. AddMedicationDrawer (1218 LOC, 33 useState, lógica de dose/ciclo BK-02) manteve o `useState` existente por decisão de risco aprovada por Fábio — validação via `medicationSchema.safeParse()` no `handleSave`, sem reescrever o state management sem cobertura de QA visual. `medicationSchema` espelha a CHECK constraint `chk_cyclic_fields_complete`. `signupSchema` valida `consentAccepted === true` (LGPD Art. 11) no schema. Testes unitários Vitest para os 4 schemas. (ver ID-016 em TECH_DEBT.md) |
