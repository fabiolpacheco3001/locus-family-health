...
> **Versão:** 3.5 | **Atualizado em:** 2026-07-02
>
...
| Posologia Avançada | `AddMedicationDrawer.tsx` | **4 frequency_types:** fixed_interval, specific_times, specific_days, **cyclic** |
...
│   ├── calculateNextDose.ts   # Motor de próxima dose (4 frequency_types — ver Seção 3.2)
...
└── integrations/supabase/
    ...
    └── types.ts               # ⚠️ AUTO-GERADO — ver Seção 7.8
...
│   ├── send-medication-reminders/      # Cron push — doses ±3 min BRT (suporta cyclic)
...
### 3.2 Tabela `medications` — Posologia (4 modelos)

```sql
frequency_type  TEXT    -- 'fixed_interval' | 'specific_times' | 'specific_days' | 'cyclic'
...
-- BK-02: Ciclos Posológicos (migration 20260702000000_add_cyclic_posology — aplicado 2026-07-02)
cycle_active_days   INTEGER      -- dias ativos do ciclo (ex: 21 para anticoncepcional)
cycle_pause_days    INTEGER      -- dias de pausa (ex: 7)
cycle_start_date    TIMESTAMPTZ  -- data de início do primeiro ciclo ativo
-- Os 3 campos são obrigatorios quando frequency_type = 'cyclic' (CHECK constraint no banco)
```

**Lógica cyclic — `calculateNextDose.ts`:**
- Fase determinada por `Math.floor((hoje - cycle_start_date) / MS_PER_DAY) % (active + pause)`
- Se `dayInCycle < cycleActiveDays` → fase ativa → retorna próxima dose em `specific_times`
- Se `dayInCycle >= cycleActiveDays` → **fase de pausa → retorna `null` (RISCO HIGH — nunca exibir dose)**
- `useHomeData.ts` expõe `cyclePhase: "active" | "pause" | null` para badge na Home

Ao modificar posologia: validar os **4** frequency_types e testar fase ativa E fase pausa. **Sempre normalizar** `'interval'` → `'fixed_interval'`.
...
created_by  UUID    -- FK auth.users — ownership direto (anti-IDOR)
...
- Validar os **4** `frequency_type`: `'fixed_interval'`, `'specific_times'`, `'specific_days'`, `'cyclic'`
- Normalizar legado: `rawType === 'interval' ? 'fixed_interval' : rawType`
- Para `'cyclic'`: verificar `cycle_active_days`, `cycle_pause_days`, `cycle_start_date`
- Testar fase **ativa** E fase **pausa** — retornar `null` durante pausa é comportamento correto e obrigatório
- Idempotência via constraint `UNIQUE(medication_id, scheduled_for)`
- `send-medication-reminders` já possui branch cyclic — ao modificar a edge function, não quebrar o guard de pausa
...
src/integrations/supabase/types.ts    ← ver nota abaixo ⚠️
supabase/config.toml                   ← gerenciado pelo Lovable
```

**⚠️ IMPORTANTE — Regeneração de `types.ts` no Lovable Cloud:**

O comando `supabase gen types typescript --project-id xazlrdwdkafhzwkezfxz` **NÃO FUNCIONA** neste projeto. O Lovable Cloud é proprietário da organização Supabase; o token pessoal do CLI não tem permissão para acessar a API de geração de tipos.
...
**Workflow correto para regenerar types.ts:**
1. **Automático (padrão):** Lovable regenera `types.ts` automaticamente ao aplicar migrations via MCP
2. **Verificar se colunas chegaram:** usar `query_database` via Lovable MCP
3. **Forçar atualização:** enviar `send_message` ao Lovable pedindo regeneração explícita
4. **Nunca editar `types.ts` manualmente** — é auto-gerado e será sobrescrito

**IDs de referência:**
- Supabase project ref (para referência, não funciona no CLI): `xazlrdwdkafhzwkezfxz`
...
| Handover de sessão | `docs/SESSAO_[N]_HANDOVER_[data].md` |
...
*Mantido pelo Claude (Cowork). v3.5 — adicionado BK-02 (cyclic posology, 4 frequency_types) e esclarecimento sobre regeneração de types.ts no Lovable Cloud (CLI não funciona — usar Lovable MCP).*
