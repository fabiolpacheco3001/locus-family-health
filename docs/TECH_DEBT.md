...
| Sessão 35 | BK-01 ✅ Web Push VAPID end-to-end: `public/sw.js` (push + notificationclick + pushsubscriptionchange), `src/lib/pushConfig.ts`, `src/hooks/usePushSubscription.ts` (PushManager.subscribe + upsert push_subscriptions), `src/pages/Notificacoes.tsx` (opt-in card). Edge Functions: `send-push-notification` (npm:web-push@3.6.7, auto-deactivates 410/404), `send-medication-reminders` (±3min window, 3 frequency_types, BRT), `send-appointment-reminders` (D-0 e D-1, 8h BRT). Migration `20260621120000_push_notifications.sql`: tabela, RLS, índices, TTL pg_cron dominical. pg_cron jobs ativos com `timeout_milliseconds:=30000`. Incidente: VAPID_PRIVATE_KEY exposta no GitHub → keys rotacionadas, force push, Supabase secrets atualizados. Smoke test final: `net._http_response` ID 20 → `200 {"processed":0}` ✅. | Sprint 35 ✅ CONCLUÍDO |
| Sessão 37 | Bug ∞ Dipirona ✅ (Fase 401 — causa raiz real): `frequency_type = "interval"` (valor legado gravado no banco pelas versões antigas do `AddMedicationDrawer`) não era reconhecido por `calculateNextDose` nem por `advancePastTakenDoses`, pois `"interval"` é truthy e impedia o fallback para `"fixed_interval"`, causando dois sintomas: (a) med com `frequency_hours>0` desaparecia silenciosamente do widget Home (função retornava null, era filtrado); (b) med com `frequency_hours=null` mostrava ∞ indevidamente (`isContinuous=true`). Fix: normalização `rawType === "interval" ? "fixed_interval" : rawType` em `calculateNextDose.ts` (linha ~27), `advancePastTakenDoses.ts` (linha ~38) e `useHomeData.ts` (linha ~255). Testes de regressão adicionados ao `calculateNextDose.test.ts`: 2 novos `describe` (7 novos `it`) cobrindo alias `"interval"` e `advancePastTakenDoses` com skip de dose tomada. Valores esperados validados offline via Node.js. | Sprint 37 ✅ CONCLUÍDO |
| Sessão 36 | BK-01 E2E fix ✅: diagnóstico completo do push não chegar no iPhone. Root cause: VAPID key pair inconsistente — `VAPID_PRIVATE_KEY` no Supabase Secrets não correspondia à public key em `pushConfig.ts` (geradas em momentos distintos durante o incidente de rotação da sessão 35). Evidência: campo `push_details` adicionado ao response de `send-medication-reminders` revelou `{sent:0,failed:1}` (APNs rejeitando) vs `{sent:1,failed:0}` após fix. Fix: par VAPID P-256 regenerado via Node.js WebCrypto (`subtle.generateKey` + JWK export), `pushConfig.ts` e default do `send-push-notification` atualizados com nova public key, Supabase Secrets atualizados com ambas as chaves, usuário re-subscreveu. Validação final: notificação "💊 Hora do Remédio!" recebida no iPhone (PWA fechado, app não aberto). Bug fix colateral: `send-medication-reminders` contava `sent` por `res.ok` (HTTP 200), não pelo resultado real do APNs — corrigido com leitura do response body. JSDoc com VAPID_PRIVATE_KEY removido de `send-push-notification/index.ts`. | Sprint 36 ✅ CONCLUÍDO |
