...
> **Versão:** 5.9 | **Atualizado em:** 2026-06-29 (sessão 42 — CPF guard hard-block em MeuPlano + PaywallModal)  
...
| Sessão 42 | **PROD-03 complemento — CPF guard hard-block**: Guard anterior era advisory-only (`toast.warning` sem bloqueio) — UX quebrada: tab em branco abria, edge function chamada, 422 `missing_cpf` retornado, tab ficava em branco. Fix: 3 handlers de checkout agora verificam `!hasCpf` ANTES de qualquer `window.open` → emitem `toast.info` + `navigate("/meus-dados")` + `return` early. Arquivos: `MeuPlano.tsx` (handlers `handleRegularize` + `handleReactivate`) e `PaywallModal.tsx` (handler `handleSubscribe`, adicionado `useNavigate`). Server-side guard (422 missing_cpf) mantido como defense in depth. Commit `57381b0`. | Sprint 42 ✅ CONCLUÍDO |
