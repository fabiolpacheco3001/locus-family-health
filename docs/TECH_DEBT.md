...
> **Versão:** 8.6 | **Atualizado em:** 2026-07-02 (sessão 71 — ID-016/BK-11 Zod schemas para PHI implementado; ID-021 registrado — refactor AddMedicationDrawer)  
...
|--------|-----------------|---------------|
| Sessão 71 | **ID-016 / BK-11 — Validação Zod centralizada para PHI ✅ (commit `3c72cb2`, LOCAL)**: `src/lib/schemas/{auth,consultation,surgery,medication}.ts` criados. Cadastro.tsx, Login.tsx, AddConsultationDrawer.tsx e AddSurgeryDrawer.tsx migrados para `react-hook-form` + `zodResolver`. **Decisão de escopo aprovada por Fábio (abordagem híbrida por risco):** `AddMedicationDrawer.tsx` (1218 LOC, 33 `useState`, lógica de dose/ciclo BK-02 alterada na sessão anterior) manteve o `useState` — sem cobertura de QA visual no sandbox, reescrever o state management de um arquivo que decide dose de medicamento foi avaliado como risco desnecessário. Validação via `medicationSchema.safeParse()` no `handleSave` (ambos os branches isEditing e criação single — branch de criação em lote via OCR `buildMedPayloadFromExtracted` fica fora de escopo, shape de payload distinto). `medicationSchema` espelha a CHECK constraint `chk_cyclic_fields_complete` via `superRefine` — nunca permite `frequency_type='cyclic'` sem os 3 campos de ciclo. `signupSchema` valida `consentAccepted === true` (LGPD Art. 11) no próprio schema. 4 arquivos de teste Vitest criados (`src/lib/schemas/*.test.ts`) cobrindo os 4 `frequency_type` + guard de ciclo + LGPD consent + faixa fisiológica de sistólica/diastólica (1-300mmHg) + obrigatoriedade de `custom_type` quando `surgery_type='outro'`. `npx tsc --noEmit`: 0 erros. `eslint`: sem novos warnings (3 warnings pré-existentes de `any` em blocos `catch` de checkout, não introduzidos nesta sessão). **Nota de ambiente:** `npx vitest run` falhou no sandbox por incompatibilidade de binário nativo rollup/esbuild — reproduzido também em `calculateNextDose.test.ts` (arquivo não tocado, não é regressão); lógica dos 4 schemas verificada manualmente via `node --experimental-strip-types` com resultados corretos. Recomenda-se rodar `npm run test`/`bun run test` localmente para confirmar a suíte. **⚠️ git push pendente:** commits `3c72cb2` (esta sessão) e `d4b89d8` (sessão 70, handover) LOCAL não empurrados — sandbox sem credencial de rede para GitHub; Fábio deve rodar `git push origin main` do terminal. **[ID-021 registrado — não implementado]** Fábio questionou se `AddMedicationDrawer.tsx` (1218 LOC, 33 `useState`) merece refactor. Análise: duplicação real entre `buildMedPayload`/`buildMedPayloadFromExtracted` (o fluxo OCR em lote nem suporta `cyclic`) + bloco JSX de horário/chips copiado 3x. Fábio optou por só documentar em TECH_DEBT.md agora, sem tocar no arquivo — evita empilhar risco de refactor estrutural sobre a mudança de validação Zod ainda em teste manual. | ID-016/BK-11 ✅ CONCLUÍDO / ID-021 ⬜ BACKLOG |
| Sessão 70 | **BK-02 — Ciclos Posológicos Complexos ✅ (commit `1d767fe`, Lovable MCP `d65156602`):** Novo `frequency_type = 'cyclic'` para suportar anticoncepcional 21+7 e similares. **Risco HIGH:** nunca exibir dose durante fase de pausa. Migration `20260702000000_add_cyclic_posology.sql`: 3 novas colunas nullable (`cycle_active_days INT`, `cycle_pause_days INT`, `cycle_start_date TIMESTAMPTZ`) + CHECK constraint que garante preenchimento obrigatório dos 3 campos quando `frequency_type = 'cyclic'`. **5 arquivos frontend (LOCAL):** `calculateNextDose.ts` — branch cyclic com aritmética modular `Math.floor(daysSinceStart) % cycleTotal`, helper `getCyclePhase`, helper `nextCycleActiveStart`; `useMedications.tsx` — tipos `Medication/NewMedication/UpdateMedication` com `cycle_*` fields; `AddMedicationDrawer.tsx` — opção "Ciclo (com pausa)", defaults 21+7, UI amber com `AlertTriangle`, sumário dinâmico "X dias com dose + Y dias de pausa"; `AdherenceHistoryDrawer.tsx` — geração virtual de doses apenas nos dias ativos do ciclo; `useHomeData.ts` — `MedWithNextDose.cyclePhase: "active"\|"pause"\|null`, badge "Pausa do Ciclo" na Home, filtro sempre exibe meds em pausa. **Edge Function (Lovable MCP):** `send-medication-reminders` — branch `cyclic` bloqueia push nos dias de pausa (segurança ANVISA), envia "🔄 Reinício do Ciclo Amanhã" no último dia de pausa com tag diferenciada. `src/integrations/supabase/types.ts` — regenerar após pull com `supabase gen types typescript --project-id [ID] > src/integrations/supabase/types.ts`. **⚠️ git push pendente:** commit `1d767fe` LOCAL não empurrado (sandbox sem rede); Fábio deve `git pull --rebase origin main && git push origin main` do terminal. | BK-02 ✅ CONCLUÍDO |
...
- **Status:** ✅ Resolvido (sessão 71, 2026-07-02, commit `3c72cb2`) — `src/lib/schemas/{auth,consultation,surgery,medication}.ts` criados. **Decisão de escopo (aprovada por Fábio):** Cadastro.tsx, Login.tsx, AddConsultationDrawer.tsx e AddSurgeryDrawer.tsx migrados integralmente para `react-hook-form` + `zodResolver`. `AddMedicationDrawer.tsx` (1218 LOC, 33 `useState`, lógica de dose/ciclo BK-02 alterada na sessão anterior) manteve o gerenciamento de estado atual — reescrever para RHF sem cobertura de QA visual em um arquivo que decide dose de medicamento foi avaliado como risco desnecessário. Validação obtida via `medicationSchema.safeParse()` no `handleSave`, cobrindo os 4 `frequency_type` e espelhando a CHECK constraint `chk_cyclic_fields_complete` (nunca permite `cyclic` sem os 3 campos de ciclo). `signupSchema` valida `consentAccepted === true` (LGPD Art. 11) no schema, não apenas via UI. Testes unitários Vitest cobrindo os 4 schemas (`src/lib/schemas/*.test.ts`). `npx tsc --noEmit` limpo, `eslint` sem novos warnings. **Nota de ambiente:** `npx vitest run` falhou no sandbox de execução por incompatibilidade de binário nativo do rollup/esbuild (reproduzido também em `calculateNextDose.test.ts`, arquivo não tocado — não é regressão desta sessão); lógica dos 4 schemas verificada manualmente via `node --experimental-strip-types`. Recomenda-se rodar `npm run test` (ou `bun run test`) no terminal local/CI para confirmar a suíte Vitest.
...
### ID-021 · `AddMedicationDrawer.tsx` — 1218 LOC, 33 `useState`, lógica duplicada entre payloads
- **Risco:** Componente concentra 3 fluxos distintos (criação simples, edição, wizard/OCR multi-medicamento) num único arquivo com 33 `useState`. Dois sintomas concretos de manutenibilidade: (1) `buildMedPayload()` (linha ~445) e `buildMedPayloadFromExtracted()` (linha ~488) duplicam quase toda a lógica de transformação de `frequency_type` → payload — mudança na lógica de frequência precisa ser replicada manualmente nos dois lugares, e hoje já estão inconsistentes (`buildMedPayloadFromExtracted` não suporta `frequency_type: 'cyclic'`, então o fluxo de OCR em lote nunca gera medicamento cíclico mesmo que a IA extraia esse padrão de uma receita). (2) O bloco JSX "input de horário + chips removíveis" é copiado quase identicamente 3 vezes (`specific_times`, `cyclic`, `specific_days` — linhas ~852, ~934, ~1011), sem componentização.
- **Arquivos:** `src/components/AddMedicationDrawer.tsx`
- **Fix proposto (staged, não big-bang):** (1) Unificar `buildMedPayload`/`buildMedPayloadFromExtracted` numa função pura compartilhada (parametrizada pela fonte dos dados), eliminando a divergência de suporte a `cyclic`; (2) extrair o bloco repetido de horário/chips para um subcomponente `TimePickerChips`; (3) só depois, avaliar separar os 3 fluxos (single/edit/wizard) em componentes distintos — este último passo tem risco maior e deve vir com E2E de cobertura antes de mexer no `useState`.
- **Severidade × Esforço:** 🟡 Melhoria × Alto (arquivo crítico de dose/ciclo, sem E2E de regressão hoje — requer sessão dedicada com testes escritos antes da refatoração)
- **Status:** ⬜ Backlog — identificado na sessão 71 (2026-07-02) durante revisão pós-ID-016/BK-11. Fábio optou por **apenas registrar o débito nesta sessão**, sem tocar no arquivo agora — decisão explícita para não empilhar risco de refactor estrutural em cima da mudança de validação Zod ainda em teste manual.
...
├── ✅ BK-02                             → Ciclos posológicos complexos (anticoncepcional 21+7) — sessão 70
